...
ws2811_led_t rgb_to_led_color(unsigned char red,
                              unsigned char green,
                              unsigned char blue) {
...
void on_connect() {
  ...
  client.socket()->on("sign reset buffer",
  ...
  client.socket()->on("sign receive routine from server",
           sio::socket::event_listener_aux([&](
               std::string const& name,
               sio::message::ptr const& data,
               bool isAck,
  ...
  client.socket()->on("sign receive pixel from server",
           sio::socket::event_listener_aux([&](
               std::string const& name,
               sio::message::ptr const& data,
               bool isAck,
  ...
  client.socket()->on("apply to sign",
           sio::socket::event_listener_aux([&](
               std::string const& name,
               sio::message::ptr const& data,
               bool isAck,
  ...
  while(true) {
    if(!play) {
      if(reset) {
        ...
        for(size_t current_pixel = 0;current_pixel < 300;
            current_pixel++) {
          char r = 10 + (current_pixel * 26 + i * 24) % 20;
          char g = 0 + (current_pixel * 15 + i * 10) % 1;
          char b = 0 + (current_pixel * 4 + i * 5) % 1;

          ledstring.channel[0].leds[current_pixel]
            = rgb_to_led_color(r, g, b);
        }
    ...
    while(!reset && play) {
      for(size_t current_routine = 0;current_routine < routines.size() && play
        ...
        for(size_t current_cycle = 0;current_cycle < (size_t)cycles && play
              && !reset;
            current_cycle++) {
          ...
          for(size_t current_frame = 0;
              current_frame < routines_tmp[current_routine].pixels[0].r.size()
                && play && !reset;
              current_frame++) {
            std::cout << "running routine " << current_routine
              << " cycle " << current_cycle
              << " frame " << current_frame
              << std::endl;
            ...
            while(slept < interval && play && !reset) {
              i++;
              for(size_t current_pixel = 0;current_pixel < 150 && play && !reset;
                  current_pixel++) {
                if(i > 1000000) i = 0;

                int next_frame = (current_frame + 1) % routines_tmp[current_routine].pixels[0].r.size(); 
                char r = mix(routines_tmp[current_routine].pixels[current_pixel]
                    .r[current_frame], routines_tmp[current_routine].pixels[current_pixel]
                    .r[next_frame], slept, interval);
                char g = mix(routines_tmp[current_routine].pixels[current_pixel]
                    .g[current_frame], routines_tmp[current_routine].pixels[current_pixel]
                    .g[next_frame], slept, interval);
                char b = mix(routines_tmp[current_routine].pixels[current_pixel]
                    .b[current_frame], routines_tmp[current_routine].pixels[current_pixel]
                    .b[next_frame], slept, interval);

                for(size_t pair = 0;pair < 2;pair++) {
                  ledstring.channel[0].leds[current_pixel * 2 + pair]
                    = rgb_to_led_color(
                        std::min(255, std::max(0, (int)(r * ((current_pixel + i) % spread) / spread))) * brightness,
                        std::min(255, std::max(0, (int)(g * ((current_pixel + i) % spread) / spread))) * brightness,
                        std::min(255, std::max(0, (int)(b * ((current_pixel + i) % spread) / spread))) * brightness);
                }
              }
